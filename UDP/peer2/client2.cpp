    ...
    while(1)
    {
        ...
        // signal(SIGINT, sigHandler);
        {
            ...
            else if(!strcmp(start,"IndexGet"))
            {
                ...
                sendto(sock, start , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));

                scanf("%s", buf);
                //-------------write(sock, buf, 1024);
                sendto(sock, buf , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                if(!strcmp(buf, "LongList"))
                {
                    //-------read(sock, &recv_data_int, 1024);
                    recvfrom(sock, &recv_data_int , sizeof(int),0, (struct sockaddr *)&server_addr, &addr_len);
                    for(int i=0;i<recv_data_int-1;i++)
                    {
                        char file[1024];
                        bytes_recieved = recvfrom(sock, file, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                ...
                else if(!strcmp(buf,"ShortList"))
                {
                    ...
                    scanf("%s",t1);
                    // --------write(sock, t1, 1024);
                sendto(sock, t1 , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                    scanf("%s",t2);
                // ------    write(sock, t2, 1024);
                sendto(sock, t2 , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                    //-----------read(sock, &recv_data_int, 1024);
                      recvfrom(sock, &recv_data_int, sizeof(int),0, (struct sockaddr *)&server_addr, &addr_len);
                ...
                sendto(sock, regex , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
            ...
            else if(!strcmp(start,"FileHash"))
            {
                ...
                {
                // ---    write(sock, ver, 1024);
                sendto(sock, ver , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                    scanf("%s",buf2 );
                    strcpy(copy, buf2);
                    printf("Name:%s\n",buf2);
                // ---    write(sock , buf2, 1024);
                sendto(sock, buf2 , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));

                    //---------bytes_recieved = read(sock, buf5, 1024);
                        bytes_recieved = recvfrom(sock, buf5, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                    buf5[bytes_recieved]='\0';
                    if(!strcmp(buf5, "Error"))
                    {
                        printf("No such file exits in other peer.\n");
                    }
                    else
                    {
                        strcpy(copy, buf2);
                        char oldmd5[1024];
                        strcpy(oldmd5, buf5);
                        printf("The MD5 value in other client is: %s\n",oldmd5);

                        //------------bytes_recieved = read(sock, buf3, 1024);
                        bytes_recieved = recvfrom(sock, buf3, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                        buf3[bytes_recieved]='\0';
                        printf("Date is :%s\n", buf3);

                        //--------bytes_recieved = read(sock, buf4, 1024);
                        bytes_recieved = recvfrom(sock, buf4, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                ...
                // ----            write(sock, "OK", 1024);
                sendto(sock, "OK" , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                        }
                        else
                        {
                            strcpy(copy, buf2);
                ...
                sendto(sock, "NOK" , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                        ...
                        bytes_recieved = recvfrom(sock, data, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                ...
                sendto(sock, &l , sizeof(int), 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                    string newline[l];
                    for(int i=0;i<l;i++)
                    {
                ...
                sendto(sock, file , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                            strcpy(copy, file);
                            //-----bytes_recieved = read(sock, buf5, 1024);
                        bytes_recieved = recvfrom(sock, buf5, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                            buf5[bytes_recieved]='\0';
                ...
                sendto(sock, "NOK" , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                                    ...
                                    //-----------read(sock,&din,sizeof(int));
                                     recvfrom(sock, &din, sizeof(int),0, (struct sockaddr *)&server_addr, &addr_len);
                                    //----------bytes_recieved = read(sock,data,1024);
                        bytes_recieved = recvfrom(sock, data, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                                    data[bytes_recieved]='\0';
                                    //printf("\n");
            ...
            else if(!strcmp(start,"FileUpload"))
            {
                ...
                sendto(sock, &upload , sizeof(int), 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                strcpy(copy,buf2);
                //---------------bytes_recieved = read(sock, buf, 1024);
                        bytes_recieved = recvfrom(sock, buf, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                ...
                sendto(sock, forsbuf , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                ...
                sendto(sock, buf5 , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                ...
                sendto(sock, buf3 , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                        date.close();
                        remove("date");
                ...
                sendto(sock, buf4 , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                        time.close();
                        remove("time");
                ...
                sendto(sock, fbuf , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                        }
                //------        write(sock , &recv_data_int, sizeof(int));
                sendto(sock, &recv_data_int , 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                //-------        write(sock , "Over", 1024);
                sendto(sock, "Over", 1024, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
                        fclose(fd);
                    }
            ...
            else if(!strcmp(start,"FileDownload"))
            {
                ...
                //-----------bytes_recieved=read(sock, forsbuf ,1024);
                        bytes_recieved = recvfrom(sock, forsbuf, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
                ...
                else
                {
                    ...
                    //----------bytes_recieved = read(sock,buf2,1024);
                        bytes_recieved = recvfrom(sock, buf2, 1024,0, (struct sockaddr *)&server_addr, &addr_len);
