...
int main()
{
    ...
    char file_size[1024];
    printf("UDP Server Waiting for client on port 5001---\n");
    fflush(stdout);
    ...
    {  
            ...
            bytes_recieved = recvfrom(sock, start, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
            ...
            if(!strcmp(start1, "IndexGet"))
            {
                //--------bytes_recieved = read(connected,buf,1024);
                        bytes_recieved = recvfrom(sock, buf, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
               ...
               // printf("seconf: %s\n", buf);
                if(!strcmp(buf, "LongList"))
                {
                  //  printf("In\n");
                    ...
                    sendto(sock, &l, sizeof(int), 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                   ...
                   // write(connected, &l, sizeof(int));
                    ...
                    sendto(sock, file, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                            strcpy(copy, file);
                    ...
                    sendto(sock, line1.c_str(), 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                    ...
                    sendto(sock, line2.c_str(), 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                    ...
                    sendto(sock, line3.c_str(), 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                ...
                else if(!strcmp(buf, "ShortList"))
                {
                    ...
                    ///---------bytes_recieved = read(connected, t1 ,1024);
                        bytes_recieved = recvfrom(sock, t1, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                    t1[bytes_recieved] = '\0';
                    //printf("first t1: %s\n",t1);
                    strcpy(t11,t1);
                    //---------bytes_recieved = read(connected, t2 ,1024);
                        bytes_recieved = recvfrom(sock, t2, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                    t2[bytes_recieved] = '\0';
                    //printf("first t2: %s\n",t2);
                    strcpy(t22,t2);
                    ...
                    sendto(sock, "No", 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                            continue;
                        }
                        else
                        {
                ...
                else if(!strcmp(buf, "RegEx"))
                {
                    ...
                    //--------bytes_recieved = read(connected, regex ,1024);
                        bytes_recieved = recvfrom(sock, regex, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                    ...
                    sendto(sock, newline[i].c_str(), 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
            ...
            else if(!strcmp(start1, "FileHash"))
            {
                ...
                //----------bytes_recieved = read(connected,ver,1024);
                        bytes_recieved = recvfrom(sock, ver, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                ver[bytes_recieved] = '\0';
        //        printf("Itis: %s\n", ver);
                if(!strcmp(ver,"Verify"))
                {
                    //----------bytes_recieved = read(connected, buf2,1024);
                        bytes_recieved = recvfrom(sock, buf2, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
          ...
          //          printf("--%s\n", buf2);
                    ...
                    sendto(sock, "Error", 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                    }
                    else
                    {
                        //fclose(fd);
                    ...
                    sendto(sock, buf5, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                    ...
                    sendto(sock, buf3, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                        date.close();
                        remove("date");
                    ...
                    sendto(sock, buf4, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                        time.close();
                        remove("time");
                        ...
                        bytes_recieved = recvfrom(sock, chek, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                    ...
                    sendto(sock, data, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                            }
                            //       printf("---------------------------------------------------------------------------------------------------------------\n");
                            //---------write(connected, &recv_data_int, sizeof(int));
                    sendto(sock, &recv_data_int, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                            //----------write(connected, "Over", 1024);
                    sendto(sock, "Over", 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                            fclose(fd);
                ...
                else if(!strcmp(ver,"CheckAll"))
                {
                    ...
                    recvfrom(sock, &recv_data_int, sizeof(int),0, (struct sockaddr *)&client_addr, &addr_len);
                    char file[1024];
            //        printf("limit is:%d\n", recv_data_int);
                    for(int i=0;i<recv_data_int;i++)
                    {
                        //-------bytes_recieved = read(connected ,file ,1024);
                        bytes_recieved = recvfrom(sock, file, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
            ...
            else if(!strcmp(start1,"FileUpload"))
            {
             ...
             //   printf("A request from uploading a file from the client:\nType: \nFileUpload Allow: to allow.\nFileUpload Deny: to deny.\n");
                ...
                //---------read(connected, &upload, sizeof(int));
                    recvfrom(sock, &upload, sizeof(int),0, (struct sockaddr *)&client_addr, &addr_len);
               // printf("upload: %d\n ", upload);
                if(upload == 0)
                {
                    printf("Allowing to upload......\n");
                    strcpy(buf, "FileUpload Allow");
                }
                else
                {
                    printf("Denying to upload.........\n");
                    strcpy(buf, "FileUpload Deny");
                }
                //------write(connected, buf, 1024);
                    sendto(sock, buf, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
               // printf("so it is:--%s\n", buf);
                if(!strcmp(buf,"FileUpload Allow"))
                {
                    //---------bytes_recieved = read(connected,buf2,1024);
                        bytes_recieved = recvfrom(sock, buf2, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                    buf2[bytes_recieved] = '\0';
                ...
                //    printf("Name of File Uploaded: %s\n",buf2);
                    ...
                    //---------bytes_recieved = read(connected,forsbuf,1024);
                        bytes_recieved = recvfrom(sock, forsbuf, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                    forsbuf[bytes_recieved] = '\0';
                  //  printf("Size of that file: %s \n", forsbuf);

                    //--------bytes_recieved = read(connected,buf5,1024);
                        bytes_recieved = recvfrom(sock, buf5, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                    buf5[bytes_recieved] = '\0';
                  //  printf("MD5 hash of that file: %s \n", buf5);

                    //-------bytes_recieved = read(connected,buf3,1024);
                        bytes_recieved = recvfrom(sock, buf3, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                    buf3[bytes_recieved] = '\0';
                  //  printf("Last Modified Date: %s\n", buf3);

                    //------bytes_recieved = read(connected,buf4,1024);
                        bytes_recieved = recvfrom(sock, buf4, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                  ...
                  //  printf("Last Modified Time: %s \n",buf4);
                    ...
                    //------bytes_recieved = read(connected, fbuf ,1024);
                        bytes_recieved = recvfrom(sock, fbuf, 1024,0, (struct sockaddr *)&client_addr, &addr_len);
                ...
                else if(!strcmp(buf, "FileUpload Deny"))
                {
            ...
            else if(!strcmp(start1,"FileDownload"))
            {
               ...
               //     printf("\nNo such file exits\n");
                ...
                else
                {
                    ...
                    sendto(sock, forsbuf, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                  //  printf("size :%s\n",forsbuf); 
                    size.close();
                    remove("size");
                    // File
                    ...
                    sendto(sock, &count, sizeof(int), 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                        //--------write(connected, buf2, 1024);
                    sendto(sock, buf2, 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                      //           printf("---------------------------------------------------------------------------------------------------------------\n");
                    }
                    //       printf("---------------------------------------------------------------------------------------------------------------\n");
                    //------write(connected, &x, sizeof(int));
                    sendto(sock, &x, sizeof(int), 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                    //---write(connected, "Over", 1024);
                    sendto(sock, "Over", 1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
                    fclose(fd);
